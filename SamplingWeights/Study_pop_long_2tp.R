#' This script runs both "long" scripts with reduced number of timepoints (n_t=2)
...
cut <- median(KoCo_BLab$week)

# Suggested cut-off points
KoCo_BLab$time_point <- cut(KoCo_BLab$week, breaks = c(15, cut , 24),
                            right = TRUE,
                            include.lowest = TRUE)
...
rm(list = setdiff(ls(), c("KoCo_BLab_split_study_pop", "cut")))
...
calib_weights <- list(w_hh_cal_logit_1, w_hh_cal_logit_2)
g_weights <- list(g_logit_1, g_logit_2)

calib_weights <- lapply(1:2, function(x){
  res <- data.frame(hh_id = names(d_house[[x]]), w_hh_cal = calib_weights[[x]],
                    g = g_weights[[x]])
})

KoCo_BLab_split_study_pop <- lapply(1:2, function(x){
...
for(i in 1:length(KoCo_BLab_split_study_pop)){
  ...
  ### Export results
  write.csv(results, here_koco_data(paste("Estimates_study_pop_t", paste(paste(i, cut, sep=""), ".csv", sep=""), sep="")), row.names = FALSE)
  ...
  ### Export results
  write.csv(results_adj, here_koco_data(paste("Estimates_study_pop_adjusted_t", paste(paste(i, cut, sep=""), ".csv", sep=""), sep="")), row.names = FALSE) 
}
