...
plt_vc_distr_igg = plt_up + annotation_custom(
  ggplotGrob(plt_down + theme(legend.position = "none")),
  ymin=0.15, ymax=0.7, xmin=0.3, xmax=3.7)
plt_vc_distr_igg
...
## Primary distributions (Figure 2)
...
# Save plot
for (fmt in style$output_formats) {
  ggsave(here_out(paste0("Fig2_Primary_Distribution.", fmt)), device=fmt,
         dpi=style$dpi, width=12, height=6, units="in")
}
```
...
plot_grid(
  plot_grid(
    title_plot("A Performance comparison of primary tests", plot_grid(iga.igg, roche.igg)),
...
# Save plot
for (fmt in style$output_formats) {
  ggsave(here_out(paste0("Fig4_Primary_Comparison.", fmt)), device=fmt,
         dpi=style$dpi, width=12, height=6, units="in")
}
```
...
plot_grid(
  plot_grid(
    ...
    title_plot("B Distribution of VC-IgG and MG", plot_grid(
      plt_vc_distr_igg + theme(legend.position="none"), plt_lineblot_distr + theme(legend.position="none"),
      get_legend(plt_vc_distr_igg), rel_widths=c(2,2,1), nrow=1)),
    rel_widths=c(4,11)),
  plot_grid(
    ...
    title_plot("D Parallel coordinate plots for VC-IgG and MG", plot_grid(
      plp_vc_igg + theme(legend.position="none"), plp_lineblot + theme(legend.position="none"),
      get_legend(plp_vc_igg), rel_widths=c(2,2,1), nrow=1)),
    rel_widths=c(4,11)),
  title_plot("E Concordance of confirmatory tests and ground truth", plt_test_gt_bars_conf),
  ncol=1, rel_heights = c(2,2,1.5)
)

# Save plot
for (fmt in style$output_formats) {
  ggsave(here_out(paste0("Fig5_Confirmatory_Distribution.", fmt)), device=fmt,
         dpi=style$dpi, width=12, height=9, units="in")
}
```
...
# Save plot
for (fmt in style$output_formats) {
  ggsave(here_out(paste0("FigSupp9_Cold.", fmt)), device=fmt,
...
# Save plot
for (fmt in style$output_formats) {
  ggsave(here_out(paste0("FigSupp1_Replicates_Scatter.", fmt)), device=fmt,
...
# Save plot
for (fmt in style$output_formats) {
  ggsave(here_out(paste0("FigSupp4_VC.", fmt)), device=fmt,
         dpi=style$dpi, width=12, height=10, units="in")
}
```
...
# Save plot
for (fmt in style$output_formats) {
  ggsave(here_out(paste0("FigSupp5_GroundTruth_All_Split_Accordance.", fmt)), device=fmt,
         dpi=style$dpi, width=12, height=4, units="in")
}
```
