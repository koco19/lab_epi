...
if (!exists("KoCo_BLab_split")) {                 
source(here_weights("Sampling_weights_long.R"))
}
...
KoCo19 <- read.csv(here_koco_data("Analysis Data Sets", "KoCo19_BaselineLAB.csv"), stringsAsFactors = F) 
...
margins_age_sex <- read.csv(here_koco_data("Analysis Data Sets", "muc_age_structure_KoCo_bis.csv"), stringsAsFactors = F)  
...
data_house_split <- lapply(KoCo_BLab_split, function(subdata){
  ...
  subdata$age_cat <- cut(subdata$Age, c(0, 19, 34, 49, 64, 80, 150),
                           include.lowest = TRUE,
                           right = TRUE,
...
d_house <- lapply(1:length(KoCo_BLab_split), function(x){
  res <- KoCo_BLab_split[[x]]$w_constituency * KoCo_BLab_split[[x]]$w_house_time
  names(res) <- KoCo_BLab_split[[x]]$hh_id
...
### Time point 1
g_trunc_1 <- calib(data_house_split[[1]], d = d_house[[1]], totals, method = "truncated",
           bounds = c(0.1, 5), max_iter = 1000)
w_hh_cal_trunc_1 <- g_trunc_1 * d_house[[1]]

# Checking if calibrated weights sum up to auxiliary data
sum(t(w_hh_cal_trunc_1) %*% as.matrix(data_house_split[[1]]) - totals[])
summary(w_hh_cal_trunc_1)
plot(density(g_trunc_1))

### Time point 2
g_trunc_2 <- calib(data_house_split[[2]], d = d_house[[2]], totals, method = "truncated",
                 bounds = c(0.2, 5), max_iter = 1000)
w_hh_cal_trunc_2 <- g_trunc_2 * d_house[[2]]

# Checking if calibrated weights sum up to auxiliary data
sum(t(w_hh_cal_trunc_2) %*% as.matrix(data_house_split[[2]]) - totals[])
summary(w_hh_cal_trunc_2)
plot(density(g_trunc_2))
sd(w_hh_cal_trunc_2)

d_house[[2]][which(w_hh_cal_trunc_2 == max(w_hh_cal_trunc_2))]

### Time point 3
g_trunc_3 <- calib(data_house_split[[3]], d = d_house[[3]], totals, method = "truncated",
                 bounds = c(0.2, 6), max_iter = 1000)
w_hh_cal_trunc_3 <- g_trunc_3 * d_house[[3]]

# Checking if calibrated weights sum up to auxiliary data
sum(t(w_hh_cal_trunc_3) %*% as.matrix(data_house_split[[3]]) - totals[])
summary(w_hh_cal_trunc_3)
sd(w_hh_cal_trunc_3)

### Time point 4
g_trunc_4 <- calib(data_house_split[[4]], d = d_house[[4]], totals, method = "truncated",
                 bounds = c(0.1, 12), max_iter = 1000)
w_hh_cal_trunc_4 <- g_trunc_4 * d_house[[4]]

# Checking if calibrated weights sum up to auxiliary data
sum(t(w_hh_cal_trunc_4) %*% as.matrix(data_house_split[[4]]) - totals[])
summary(w_hh_cal_trunc_4)
sd(w_hh_cal_trunc_4)
plot(density(g_trunc_4))

calib_weights <- list(w_hh_cal_trunc_1, w_hh_cal_trunc_2, w_hh_cal_trunc_3, w_hh_cal_trunc_4)
g_weights <- list(g_trunc_1, g_trunc_2, g_trunc_3, g_trunc_4)
...
KoCo_BLab_split <- lapply(1:4, function(x){
  res <- merge(KoCo_BLab_split[[x]], calib_weights[[x]], by = "hh_id")
...
test <- lapply(KoCo_BLab_split, function(x){
...
for(i in 1:length(KoCo_BLab_split)){
  KoCo_BLab <- KoCo_BLab_split[[i]]
  ...
  ### Export results
  write.csv(results, here_koco_data(paste("Estimates_t", paste(i, ".csv", sep=""), sep="")), row.names = FALSE)
  ...
  ### Export results
  write.csv(results_adj, here_koco_data(paste("Estimates_adjusted_t", paste(i, ".csv", sep=""), sep="")), row.names = FALSE)
}