...
KoCo19 <- read.csv(here_data("KoCo19_BaselineLAB.csv"), stringsAsFactors = F)
...
margins_age_sex <- read.csv(here_data("muc_age_structure_KoCo_bis.csv"), stringsAsFactors = F)
...
# KoCo_BLab$w_ind_cal_igg <- KoCo_BLab$w_ind_cal
# KoCo_BLab$w_ind_cal_igg[KoCo_BLab$ind_id == "2866P02"] <- KoCo_BLab$w_ind_cal[KoCo_BLab$ind_id == "2866P02"] * 2
# KoCo_BLab$w_ind_cal_igg[KoCo_BLab$ind_id == "2866P01"] <- NA
...
KoCo_BLab$age_cat <- cut(KoCo_BLab$Age, c(0, 19, 34, 49, 64, 80, 150),
                            include.lowest = TRUE,
                            right = TRUE,
...
# Reordering the columns
data_house_igg <- data_house_igg[, c("hh_id", "Male_<=19", "Male_20-34", "Male_35-49", "Male_50-64",
                                     "Male_65-79", "Male_>=80", "Female_<=19", "Female_20-34", 
...
### Which method to use between trunc and logit? 
# The one with the lowest variance of the weights
sd(w_hh_cal_trunc)
sd(w_hh_cal_logit)

calib_weights <- data.frame(hh_id = names(d_house), w_hh_cal = w_hh_cal_trunc,
                            g = g_trunc)
...
### method "linear"
g <- calib(data_house_igg, d = d_house, totals, method = "linear")
w_hh_cal_lin <- g * d_house
...
### method "raking"
g <- calib(data_house_igg, d = d_house, totals, method = "raking")
w_hh_cal_rak <- g * d_house
...
calib_weights <- data.frame(hh_id = names(d_house), w_hh_cal_igg = w_hh_cal_trunc,
                            g_igg = g_trunc)
...
# Removing temporary data
rm(list = setdiff(ls(), c("here_data", "here_statistics", "KoCo_BLab", "data_house", "Const", "Munich_hh")))
...
# Remove one hh too much
# hh_u_k <- hh_u_k[, colnames(hh_u_k) %in% KoCo_BLab$hh_id]
...
### Export results
write.csv(results, here_data("Estimates.csv"), row.names = FALSE)
...
### Export results
write.csv(results_adj, here_data("Estimates_adjusted.csv"), row.names = FALSE)
