...
for(i in 1:length(KoCo_BLab_split_study_pop)){
  ...
  ### Export results
  write.csv(results, here_koco_data(paste("Estimates_study_pop_t", paste(i, ".csv", sep=""), sep="")), row.names = FALSE)
  ...
  ### Export results
  write.csv(results_adj, here_koco_data(paste("Estimates_study_pop_adjusted_t", paste(i, ".csv", sep=""), sep="")), row.names = FALSE) 
}