---
title: "Lab Paper 2nd part (updated plots for 2021)"
author: "Mercè Garí"
output:
  html_document:
    df_print: paged
...
here_out = function (...) here::here(
  "Lab_paper2_output", "lab_figures_merce_updated21", ...)
...
thresholds$Manufacturer[thresholds$Test == "NT"] <- 5
str(thresholds)
...
no.se.que <- lab %>%
  ...
  mutate(quadrant = case_when(
    value < Optimized & NT == "<5" ~ "<5-below",
    value >= Optimized & NT == "<5" ~ "<5-above",
     value < Optimized & NT == "1:5" ~ "1:5-below",
    value >= Optimized & NT == "1:5" ~ "1:5-above",
...
p1 <- lab %>%
  ...
  theme(legend.position="none") +
   geom_text(data = filter(counts, quadrant == "10-below" & 
                            `Ground truth` == "True-positive"),
            aes(x=3.15, y=-50, label=paste(round(Percentage, 0), "%")),
            hjust=1, vjust=1, show.legend=FALSE, size=3.5) +
  ...
  geom_text(data = filter(counts, quadrant == ">80-above" & 
                            `Ground truth` == "True-positive"),
            aes(x=6.15, y=130, label=paste(round(Percentage, 0), "%")),
            hjust=1, vjust=1, show.legend=FALSE, size=3.5) +
  geom_text(data = filter(counts, quadrant == "<5-below" & 
                            `Ground truth` == "True-positive"),
  ...
  geom_text(data = filter(counts, quadrant == "<5-below" & 
   ...
   geom_text(data = filter(counts, quadrant == "<5-above" & 
                            `Ground truth` == "True-positive"),
            aes(x=1.15, y=130, label=paste(round(Percentage, 0), "%")),
            hjust=1, vjust=1, show.legend=FALSE, size=3.5) +
  geom_text(data = filter(counts, quadrant == "1:5-above" & 
                            `Ground truth` == "True-positive"),
            aes(x=2.15, y=130, label=paste(round(Percentage, 0), "%")),
            hjust=1, vjust=1, show.legend=FALSE, size=3.5) +
  annotate("text", x=c(1:6), y=140, size=3,
           label=c(paste("n=", cwn$n.total))) +
  ylab("GS-cPass") +
  scale_x_discrete(labels=c("<5", "5", "10", "20", "40", ">80"))
...
p2 <- lab %>%
  ...
  geom_text(data = filter(counts, quadrant == "1:5-above" & 
    ...
    geom_text(data = filter(counts, quadrant == "10-below" & 
                            `Ground truth` == "True-positive"),
            aes(x=3.15, y=0.15, label=paste(round(Percentage, 0), "%")),
            hjust=1, vjust=1, show.legend=FALSE, size=3.5) +
  geom_text(data = filter(counts, quadrant == "10-above" & 
                            `Ground truth` == "True-positive"),
  ...
  geom_text(data = filter(counts, quadrant == ">80-above" & 
                            `Ground truth` == "True-positive"),
            aes(x=6.15, y=25, label=paste(round(Percentage, 0), "%")),
            hjust=1, vjust=1, show.legend=FALSE, size=3.5) +
   annotate("text", x=c(1:6), y=35, size=3,
           label=c(paste("n=", cwn$n.total))) +
  ylab("MG-RBD") +
  scale_x_discrete(labels=c("<5", "5", "10", "20", "40", ">80"))
p2
...
plot_grid(
  plot_grid(
    ...
    title_plot("C", p3),
    legend,
    nrow=1, rel_widths = c(0.30, 0.30, 0.30, 0.1)))

ggsave(here_out("Lab-Fig-6-updated21.png"), width=14, height=4, units="in")
ggsave(here_out("Lab-Fig-6-updated21.pdf"), width=14, height=4, units="in")
...
p4 <- lab %>%
  ...
  mutate(NT = as.character(NT),
         NT = ifelse(NT == "<5", "-30", ifelse(NT == "1:5", "5", ifelse(NT == ">80", "80", NT)))) %>%
 ...
 # scale_y_log10() +
  ...
  annotate("text", x=3.175, y=c(-30.2, 5.1, 10.1, 20.1, 40.1, 80.1), 
           label=c("<5", "5", "10", "20", "40",">80"),
...
ggsave(here_out("2_Lab-Fig-6-SI-updated21.png"), width=6, height=4, units="in")
ggsave(here_out("2_Lab-Fig-6-SI-updated21.pdf"), width=6, height=4, units="in")
```
...
p1 <- lab %>%
  ...
  ylab("GS-cPass") +
  scale_x_discrete(labels=c("<5", "5", "10", "20", "40", ">80")) +
  geom_boxplot(aes(x=as.numeric(NT)-0.05, y=cPass, group=NT), 
...
p2 <- lab %>%
  ...
  scale_y_log10(breaks=c(0.5, 1, 3, 10), labels=c("<1", "1", "3", "10")) +
  theme(legend.position="none") +
  geom_boxplot(aes(x=as.numeric(NT)-0.05, y=LineBlot_RBD_SARS_2, group=NT), 
               width=0.1, outlier.shape=NA) +
  geom_text(data = filter(counts, quadrant == "<5-below" & 
                            `Ground truth` == "True-positive"),
            aes(x=1.15, y=0.15, label=paste(round(Percentage, 0), "%")),
...
#  scale_y_log10(expand=expansion(mult=c(0.2,0.2))) + 
  ...
  geom_text(data = filter(counts, quadrant == 4 & 
            ...
            hjust=1, vjust=-1.4, show.legend=FALSE, size=3.5) +
  xlab("GS-cPass") + ylab("MG-RBD") +
    geom_smooth(data = .%>% filter(LineBlot_RBD_SARS_2 >= 1 & cPass >= 20),
              method="lm", aes(color = NULL), color="black")
p3  
...
p7a <- lab %>%
  ...
  geom_point(aes(x=as.numeric(NT)-0.25, y=IgG,
                 col=`Ground truth`),
             position = position_jitter(width = .17, ), alpha=0.5) +
   geom_boxplot(aes(x=as.numeric(NT)-0.05, y=IgG, group=NT), 
               width=0.1, outlier.shape=NA) +
  scale_color_manual(values=style$pal3) +
  ...
  geom_text(data = filter(counts, quadrant == ">80-above" & 
                            `Ground truth` == "True-positive"),
            aes(x=6.30, y=35, label=paste(round(Percentage, 0), "%")),
            hjust=1, vjust=0, show.legend=FALSE, size=3.5) +
  annotate("text", x=c(1:6), y=75, size=3,
           label=c(paste("n=", cwn$n.total))) +
...
#  scale_x_log10(expand=expansion(mult=c(0.15,0.15))) +
  ...
  ylab("EI-S1-IgG") +
  xlab("GS-cPass") +
  geom_smooth(data = .%>% filter(IgG >= 1 & value >= 20),
              method="lm", aes(color = NULL), color="black")
...
#  scale_x_log10(expand=expansion(mult=c(0.15,0.15))) +
  ...
  geom_smooth(data = .%>% filter(IgG >= 1 & value >= 1),
              method="lm", aes(color = NULL), color="black")
p7c
...
p7e <- lab %>%
  ...
  geom_point(aes(x=as.numeric(NT)-0.25, y=Roche,
                 col=`Ground truth`),
             position = position_jitter(width = .17), alpha=0.5) +
   geom_boxplot(aes(x=as.numeric(NT)-0.05, y=Roche, group=NT), 
               width=0.1, outlier.shape=NA) +
  scale_color_manual(values=style$pal3) +
  ...
  annotate("text", x=c(1:6), y=20000, size=3,
           label=c(paste("n=", cwn$n.total))) +
  ...
  geom_text(data = filter(counts, quadrant == ">80-above" & 
                            `Ground truth` == "True-positive"),
            aes(x=6.30, y=5000, label=paste(round(Percentage, 0), "%")),
            hjust=1, vjust=0, show.legend=FALSE, size=3.5) +
    scale_x_discrete(labels=c("<5", "5", "10", "20", "40", ">80"))
...
p7f <- lab.long.wo %>% filter(Test %in% c("Roche", "cPass")) %>% spread(Test, value) %>%
  ...
  ylab("Ro-N-Ig") +
  xlab("GS-cPass") +
  geom_smooth(data = .%>% filter(Roche >= 0.422 & value >= 20),
              method="lm", aes(color = NULL), color="black")
p7f
...
#  scale_x_log10(expand=expansion(mult=c(0.15,0.15))) +
  ...
  xlab("MG-RBD") +
  scale_x_log10(breaks=c(0.5,  1, 3, 10), labels=c("<1", "1", "3", "10")) +
    geom_smooth(data = .%>% filter(Roche >= 0.422 & value >= 1),
              method="lm", aes(color = NULL), color="black")
p7g
...
ggsave(here_out("Lab-Fig-7-updated21.png"), width=14, height=7, units="in")
ggsave(here_out("Lab-Fig-7-updated21.pdf"), width=14, height=7, units="in")
...
lab %>%
  mutate(NT = as.character(NT),
         NT = ifelse(NT == "1:5", "5", ifelse(NT == "<5", "1", 
                                              ifelse(NT == ">80", "80", NT)))) %>%
...
ggsave(here_out("Fig-SI-7A_updated21.pdf"), width=10, height=6)
ggsave(here_out("Fig-SI-7A_updated21.png"), width=10, height=6)
```
...
lab %>%
  mutate(NT = as.character(NT),
         NT = ifelse(NT == "<5", "1", ifelse(NT == "1:5", "5", 
                                             ifelse(NT == ">80", "80", NT)))) %>%
  mutate(NT = as.numeric(NT)) %>%
  tidyr::gather(Test, value, 
                c(IgA, cPass, NT,
...
ggsave(here_out("Fig-SI-7B_updated21.pdf"), width=10, height=6)
ggsave(here_out("Fig-SI-7B_updated21.png"), width=10, height=6)
```
...
lab.long %>%
  spread(Test, value) %>%
  mutate(NT = factor(NT, levels=c("<5", "1:5", "10", "20", "40", ">80"))) %>%
  ...
  left_join(thresholds) %>%
  filter(!is.na(NT)) %>%
  ggplot(aes(y=value, x = as.numeric(NT)-0.02, color = `Ground truth`)) +
  ...
  scale_y_log10() +
  theme(legend.position="bottom") +
  geom_boxplot(aes(x=as.numeric(NT)+0.3, y=value, group=NT), 
               width=0.1, outlier.shape=NA, show.legend=FALSE) +
  scale_x_continuous(breaks=1:6,
                     labels=c("<5", "5", "10", "20", "40", ">80")) +
  xlab("NT")
